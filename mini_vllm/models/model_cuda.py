# mini_vllm/models/model_cuda.py
# Generic transformer model - CUDA accelerated version
# Uses CUDA ops when available, falls back to PyTorch automatically
import torch
...
from mini_vllm.ops.rmsnorm import RMSNorm, fused_add_rmsnorm, fused_add_rmsnorm_plus_one
from mini_vllm.ops.rope import apply_rotary_pos_emb
from mini_vllm.ops.activation import silu, swiglu
...
class Attention(nn.Module):
    def __init__(
        ...
        # QK-norm: per-head RMSNorm on Q and K (uses CUDA op via ops.rmsnorm)
        if qk_norm:
    ...
    def forward(
        ...
        if self.num_kv_groups > 1:
            # k: [B, kv_heads, T, D] -> [B, q_heads, T, D] via expand (no data copy)
...
class DecoderLayer(nn.Module):
    def __init__(self, config: dict):
        ...
        self.rms_plus_one = rms_plus_one
        ...
        # RMSNorm from ops (auto CUDA/PyTorch)
    ...
    def forward(self, hidden_states, position_cos, position_sin, kv_cache=None, block_tables=None, slot_idx=None, layer_idx=0, use_cache=False, num_cached_tokens=0, num_cached_tokens_list=None, attn_mask=None):
        ...
        # ============================================================
        # 【优化后】Fused Add+RMSNorm：融合 Add 和 RMSNorm 为一次 kernel 调用
        # fused_add_rmsnorm 同时完成：
        #   1. residual += hidden_states（原地更新，省一次显存写入）
        #   2. output = weight * normalize(residual)
        # ============================================================
        _fused = fused_add_rmsnorm_plus_one if self.rms_plus_one else fused_add_rmsnorm
        ...
        hidden_states = self.input_layernorm(hidden_states)
        hidden_states = self.self_attn(hidden_states, position_cos, position_sin, kv_cache, block_tables, slot_idx, layer_idx, use_cache=use_cache, num_cached_tokens=num_cached_tokens, num_cached_tokens_list=num_cached_tokens_list, attn_mask=attn_mask)
        # 【优化点】融合 residual + post_attention_layernorm
        hidden_states, residual = _fused(residual, hidden_states, self.post_attention_layernorm.weight, eps)
        ...
        hidden_states = self.mlp(hidden_states)
        if self.post_feedforward_layernorm is not None:
            # 【优化点】有 post_feedforward_layernorm 时，融合 residual + layernorm
            hidden_states, residual = _fused(residual, hidden_states, self.post_feedforward_layernorm.weight, eps)
        else:
            # 【优化点】无 post_feedforward_layernorm 时，仅做 residual add
            hidden_states = residual + hidden_states
...
class TransformerModel(nn.Module):
    def __init__(self, config: dict):
        super().__init__()
        embed_scale = config.get("_embed_scale", 1.0)
