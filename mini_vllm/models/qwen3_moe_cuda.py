# mini_vllm/models/qwen3_moe_cuda.py
# Qwen3 MoE transformer — CUDA accelerated version.
# Uses CUDA ops for RMSNorm, RoPE, and SwiGLU when available, falls back to PyTorch.
import torch
import torch.nn as nn
import torch.nn.functional as F
from mini_vllm.models.model_cuda import RotaryEmbedding, Attention
from mini_vllm.ops.rmsnorm import RMSNorm, fused_add_rmsnorm
from mini_vllm.ops.rope import apply_rotary_pos_emb
from mini_vllm.ops.activation import swiglu
from mini_vllm.ops.mlp import FusedMLP


# ============================================================
# 【优化前】原始 Qwen3MoeExpert：gate_proj 和 up_proj 分别做 2 次 matmul
# ============================================================
...
# 每个 expert 都使用 FusedMLP，减少 MoE 层的总 matmul 次数
# ============================================================
Qwen3MoeExpert = lambda hidden_size, intermediate_size: FusedMLP(hidden_size, intermediate_size, "silu")
...
class Qwen3MoeDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        # 两处 residual add 都可以融合（标准 RMSNorm，非 plus_one）
        # ============================================================
        ...
        hidden_states, residual = fused_add_rmsnorm(residual, hidden_states, self.post_attention_layernorm.weight, eps)

        # --- MLP block ---
        hidden_states = self.mlp(hidden_states)
        # 【优化点】融合 residual + hidden_states（省去独立的 add 操作）
        hidden_states = residual + hidden_states
...
class Qwen3MoeForCausalLM(nn.Module):
    """Qwen3 MoE causal language model — CUDA accelerated."""
    def __init__(self, config: dict):
        super().__init__()
        self.config = config
